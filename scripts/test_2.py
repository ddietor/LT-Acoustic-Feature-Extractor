...
# %% Parameters:
datapath2save = rf".\results_{script_name.split('.')[0]}"
...
from wav_io import wav_creator
wav_creator(
    signal_uPa,
    fs,
    Nbits=Nbits,
    filename=FileName + ".wav",
    save_path=datapath2save,
    FS_uPa=FS_uPa,
    metadata_str = '{"Description":"Signal Test","Duration_s":'+str(Tdur)+',"fs_Hz":'+str(fs)+',"Nbits":'+str(Nbits)+',"FS_uPa":'+str(FS_uPa)+'}'
)
# %% wav reader:
from wav_io import wav_metadata_reader, wav_reader
metadata_str = wav_metadata_reader(os.path.join(datapath2save,FileName + ".wav"))
# print(f"Metadata readed from wav file: {metadata_str}")
Nbits_meta = metadata_str["Nbits"]
FS_meta = metadata_str["FS_uPa"]
signal_uPa_wav, fs_wav, _ = wav_reader(
    os.path.join(datapath2save, FileName + ".wav"),
    Nbits=Nbits_meta,
    FS_uPa=FS_meta
)
print(f"Max absolute value in signal_uPa: {np.max(np.abs(signal_uPa))}")
print(f"Max absolute value in signal_uPa_wav: {np.max(np.abs(signal_uPa_wav))}")
# print(f"Percentage difference: {100 * (np.max(np.abs(signal_uPa_wav)) - np.max(np.abs(signal_uPa))) / np.max(np.abs(signal_uPa)):.6f}%")
...
def spectrogram_analysis_nfft(signal,fs,NFFT,overlap,plotter=False):
    ...
    * Now the info_spect has also the overlap info: [NFFT,tbin,fbin,fvalid,overlap]
    ...
    if plotter:      
        ...
        cbar = plt.colorbar()
        cbar.set_label(r'PSD [dB re 1A$^2$/Hz]', rotation=270, verticalalignment='baseline')
        plt.xlabel('Time [s]')
        plt.tight_layout()
        plt.show()
